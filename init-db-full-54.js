...
if (fs.existsSync(dbPath)) {
    try {
        ...
        console.log('Could not delete old DB, using new name:', e.message);
        dbPath = path.join(__dirname, 'recipes-new.db');
    }
}
...
const allRecipes = [
    ...
    { id:43, slug:'spicy-chicken-feet', name_zh:'泡椒凤爪', name_en:'Spicy Chicken Feet', name_ja:'鶏足ピリ辛', name_ko:'닭발 매운맛', name_fr:'Pattes de poulet picantes', name_es:'Patas de pollo picantes' },
    ...
    { id:54, slug:'sweet-and-sour-ribs', name_zh:'糖醋排骨', name_en:'Sweet and Sour Ribs', name_ja:'酢豚排骨', name_ko:'탕수 갈비', name_fr:'Côtes aigre-douces', name_es:'Costillas agridulces' }
];
    ...
    allRecipes.forEach(r => {
        db.run('INSERT INTO recipes (id, slug, name_zh, name_en, name_ja, name_ko, name_fr, name_es) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            ...
            (err) => {
                ...
                count++;
                if (count === allRecipes.length) insertAllLevels();
            }
        );
    });
}
    ...
    for (let recipeId = 2; recipeId <= 54; recipeId++) {
        for (let level = 1; level <= 5; level++) {
            ...
            levels.push({
                ...
                st_ja: '材料洗って下処理|油でフライパン熱く|材料入れて炒め|調味料入れ|炒めて盛り付け',
                ...
                st_es:'Lavar y preparar ingredientes|Calentar aceite en la sartén|Añadir ingredientes y saltear|Añadir condimentos|Cocinar y servir',
...
function verifyData() {
    db.all('SELECT * FROM recipes', (err, recipes) => {
        ...
        db.all('SELECT * FROM levels', (err, levels) => {
            ...
            db.close(() => console.log('Done! Database initialized successfully with 54 recipes!'));
        });
    });
}
